...
This paper assumes working familiarity with the **[IASA Business Technology Architecture Body of Knowledge (BTABoK)](https://iasa-global.github.io/btabok/index.html)**. The architecture-practice vocabulary used throughout, including the competency pillars, the BIISS specializations, the proficiency levels, the CITA certifications, the managed career path, the ADLC stages, the working models, the NABC frame, and the Mentoring Method, is BTABoK's; this paper instantiates the Multiplier-Mirror framework against that vocabulary rather than re-deriving it. Readers approaching the paper without BTABoK background should consult the canonical source before Part Two, where the seven-loop cascade and the legibility crisis on CITA-layer assessment work directly against BTABoK's structure.
